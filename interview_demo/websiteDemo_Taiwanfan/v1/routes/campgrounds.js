...
router.get("/",(req,res)=>{
	...
	if(req.query.search){
		...
		if(err){console.log(err)}
			...
			if(allCampgrounds.length<1){
				let noCamps="沒有符合搜尋的地點，請再嘗試其他關鍵字。"
				return res.render("campgrounds/index",{
					campgrounds:allCampgrounds,page:'campgrounds',noCamps:noCamps,category:category})
				}
	...
	if(req.query.category){
		Campground.find({category:req.query.category},function(err,allCampgrounds){
		if(err){console.log(err)}
		else{
			res.render("campgrounds/index",
					   {campgrounds:allCampgrounds,page:'campgrounds',noCamps:noCamps,category:req.query.category})}
		})	
...
router.post("/",middleware.isLoggedIn,upload.single('image'),async (req,res)=>{
	...
	// eval(require('locus'))
	try{
		let locationCh=req.body.campground.location;
		try{
			var data= await geocoder.geocode(req.body.campground.location)
		}catch(err){
			console.log(err)
			req.flash('error', '輸入位置資訊錯誤 請再試一次');
		...
		newCampground.locationCh=locationCh
...
router.put("/:id", middleware.checkCampgroundOwnership,upload.single('image'),async(req, res)=>{
	try{
		...
		if(campground && !!campground){
			...
			req.body.campground.locationCh=locationCh;
